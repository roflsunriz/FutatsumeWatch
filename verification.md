...
- NG正規表現とフラグを`/パターン/フラグ`の1入力へ統合し、NGタグ／投稿者を一般設定のNG・フィルターへ移した。バンドラーが未使用の`new RegExp`を除去する差を検出したため、`Reflect.construct`による構文検証が実配布物にも残ること、不正2種が保存値を変えないこと、正当なpattern／flagsを両キーへ保存することを確認した。
