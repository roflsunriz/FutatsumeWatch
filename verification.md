...
## 2026-09-22：手描き案に基づく設定UI（未リリース）

- 添付された手描き案を受入基準に、設定入口を画面全体の左レールと右本文へ変更した。初期状態は左にFutatsumeWatch名・版・設定項目入口・画質・GitHub・3操作、右に「設定項目を選んでください」を表示する。「その他操作」のdetailsは廃止し、再読み込み・コメント付き画像保存・公式視聴ページを平置きした。
