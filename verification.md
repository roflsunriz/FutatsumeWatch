...
## 2026-09-22：利用可能フォントのプルダウン（未リリース）
...
- settingsスイート497項目（36設定）が成功し、手入力欄の不在、プルダウン表示、各ローカル候補の実測判定、表示された全候補の保存・再表示・元値復元、comment-overlayのコメント描画データへの反映を確認した。証跡は`dev-assets/verification/2026-09-22T05-55-08-494Z-offline-2ea24279/run.json`、画像は同ディレクトリの`settings/settings-tabs-comments-1280.png`、配布物SHA-256は`337e8f5d4960ef175e8f0f4ce841e622850db928f2b45c543412381748845669`。
- `font-options.test.ts`は利用可能候補の抽出と旧保存値の利用可否表示を固定した。公開サイト通信、権限要求、利用者環境のフォント列挙は行っていない。

## 2026-09-22：設定UIの直接切替と詳細固定時のコメント面修正（未リリース）
...
## 2026-09-22：UI案2の詳細固定とタグ閲覧専用化（未リリース）
...
## 2026-09-22：マイリスト導線の削除と大百科アイコン修正（未リリース）
