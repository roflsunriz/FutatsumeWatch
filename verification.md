...
- `test`: `bun test --preload ./test/setup.ts` が成功すること（113件 passing）。環境依存（`localStorage`・`location`・`_`・`CSS`・`console.nicoru`・`Config` の restore 待ち）は `test/setup.ts` と各テストの起動順序で吸収し、製品コード側は変えない。CDP系は `test/fixtures/cdp/scenes/*.json` を `offline.ts` で解決し、未登録・広告系は例外にして外部へ出ないことを検証する。
...
- CDP実ページ採取は雛形段階である（`scripts/cdp-capture.ts` は `chrome-debug.ps1` 起動後の手動採取用で、URL 解決の完全版と広告除外の実測は未実施）。現行シーンは既存スナップショット由来の最小再現であり、実ページ差分の取込は別途行う。
