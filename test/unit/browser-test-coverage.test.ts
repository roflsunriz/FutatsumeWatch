...
function complete(suite: BrowserSuite) {
  return {
    ...
    fields: requiredSettingsFields.map((field) => ({
      ...
      invalidInputs: ['', '0.4', '2.1', '0.55', '#xyz', '[', 'ii'],
      saved: true,
      reopened: true,
      restored: true,
    })),
...
test('必須操作を全部報告した各suiteだけを成功として数える', () => {
  ...
  expect(requiredSettingsFields).toHaveLength(84);
  expect(assertBrowserCoverage('settings', complete('settings')).settings).toBe(84);
});
...
test('設定が83件・重複・キー違いなら成功宣言でも拒否する', () => {
  const missing = complete('settings');
  missing.fields.pop();
  expect(() => assertBrowserCoverage('settings', missing)).toThrow('設定84件');
