...
// @downloadURL    https://github.com/roflsunriz/FutatsumeWatch/raw/main/dist/FutatsumeAdvancedSettings.user.js
// ==/UserScript==
/* eslint-disable */
((window) => {
    const self = window;
...
const Config = (() => {
		...
		return DataStorage.create(DEFAULT_CONFIG, {
				prefix: PRODUCT,
...
const uQuery = (() => {
		...
		const isTagLiteral = (t, ...args) => Array.isArray(t) &&
				Array.isArray(t.raw) &&
		...
		const walkingHandler = {
				...
				get: function (target, prop) {
						...
						if (!isFunc) {
								...
								const result = target.map((elm) => typeof elm[prop] === 'function'
										? elm[prop].bind(elm)
										: elm[prop]);
						...
						return (...args) => {
								const result = target.map((elm, index) => {
										try {
												const propVal = elm[prop];
												return (typeof propVal === 'function' ? propVal.apply(elm, args) : propVal) || elm;
										}
										catch (error) {
		...
		const isHTMLElement = (elm) => {
				return !!(elm instanceof HTMLElement ||
						(elm.ownerDocument &&
								elm instanceof
										elm.ownerDocument.defaultView.HTMLElement));
		};
		const isNode = (elm) => {
				return !!(elm instanceof Node ||
						(elm.ownerDocument &&
								elm instanceof elm.ownerDocument.defaultView.Node));
		};
		const isDocument = (d) => {
				const o = d;
				return !!(d instanceof Document ||
						(d && o[Symbol.toStringTag] === 'HTMLDocument') ||
						(o.documentElement && d instanceof o.documentElement.ownerDocument.defaultView.Node));
		};
		...
		const isHTMLCollection = (e) => {
				return (e instanceof HTMLCollection ||
						(e !== null && e[Symbol.toStringTag] === 'HTMLCollection'));
		};
		const isNodeList = (e) => {
				return (e instanceof NodeList ||
						(e !== null && e[Symbol.toStringTag] === 'NodeList'));
		};
		class RafCaller {
		...
		class $Array extends Array {
				...
				*getNodes() {
				...
				clone() {
						...
						this.independency.filter((e) => e.cloneNode).map((e) => e.cloneNode(true)));
				}
				find(query) {
				...
				mapQuery(map) {
						const $tmp = this.independency.filter((elm) => elm.querySelectorAll);
						const result = [], e = [], $ = {};
				...
				parents(selector) {
						const h = (selector ? this.parent().closest(selector) : this.parent());
						const found = [h];
						let current = h;
						...
						for (const element of htmls) {
								...
								for (const [k, val] of props instanceof Map ? props : Object.entries(props)) {
										...
										if (isNumber && /(width|height|top|left)$/i.test(key)) {
												value = HAS_CSSTOM ? CSS.px(value) : `${value}px`;
										}
										try {
						...
						if (callback === UNDEF) {
								for (const element of this.filter(isEventTarget)) {
										...
										for (const [key] of eventListenerMap) {
												if ((!eventKey && key.startsWith(`${elementEventName}.`)) ||
				...
				prop(key, val = UNDEF) {
						if (typeof key === 'object') {
								...
								const found = this.find((e) => Object.prototype.hasOwnProperty.call(e, key));
