...
// @downloadURL    https://github.com/roflsunriz/FutatsumeWatch/raw/main/dist/FutatsumeHLS.user.js
// ==/UserScript==
/* eslint-disable */
        ...
        const Config = (() => {
            ...
            Object.keys(DEFAULT_CONFIG).forEach((key) => {
                ...
                if (storage[storageKey]) {
                    try {
                        config[key] = JSON.parse(storage[storageKey]);
                    }
                    catch (e) {
            ...
            class IndexDBStorage {
                ...
                static async init() {
                    ...
                    return new Promise((resolve, reject) => {
                        ...
                        req.onupgradeneeded = (e) => {
                            ...
                            const store = db.createObjectStore(meta, { keyPath: 'expiresAt', autoIncrement: false });
                ...
                async load({ hash }) {
                    try {
                        ...
                        const meta = (await this.getRecord(store, hash, { index: 'hash', timeout: 3000 }));
            ...
            const Storage = {
                ...
                async load(...args) {
                    try {
                        ...
                        return { meta: result.meta, buffer: buffer };
                    }
                    catch (e) {
        ...
        const ZenzaVideoElement = (({ Hls, throttle }) => {
            ...
            const createFragmentLoader = (Hls) => {
                const frag2hash = (fragment) => {
                    ...
                    const [, videoId, level, sn] = /\/nicovideo-([a-z0-9]+)_.*\/([\d]+)\/ts\/([\d]+)\.ts/.exec(url);
                    let rel = '';
                    ...
                    return { hash: `${videoId}-${fragment.level}-${sn}${rel}`, videoId: videoId };
                };
                ...
                class FragmentLoader extends Hls.DefaultConfig.loader {
                    ...
                    async load(context, config, callbacks) {
                        ...
                        if (!context.rangeStart && !context.rangeEnd) {
                            callbacks.onSuccess = (resp, stats, context, details = null) => {
                        ...
                        callbacks.onError = (resp, context, xhr) => {
                            if (this._isAborted) {
                    ...
                    _load(context, config, callbacks, maxRetry = 3) {
                        ...
                        if (!this._isBoostTime(sn) && timeDiff < BLOCK_INTERVAL) {
                            if (maxRetry > 0) {
                                ...
                                }, Math.max(this._config.fragLoadingRetryDelay, BLOCK_INTERVAL - timeDiff));
                            }
                            else {
            ...
            const keyclean = () => {
                const keys = Object.keys(objectUrlMap).sort((a, b) => a.time - b.time);
            ...
            const overrideKeyloader = (hls) => {
                const keyloader = hls.coreComponents.find((h) => Object.prototype.hasOwnProperty.call(h, 'decryptkey'));
                if (!keyloader) {
                ...
                keyloader.loadsuccess = function (response, stats, context) {
                    ...
                    objectUrlMap[this.decrypturl] = { url: URL.createObjectURL(blob), time: Date.now() };
                    keyclean();
                    return this.loadsuccess_org(response, stats, context);
                }.bind(keyloader);
            };
          ...
          `;
                ...
                addEventListener(eventName, callback, ...options) {
                    ...
                    const wrapper = (event, ...args) => {
                        callback(overrideTarget(event, this));
                    };
                ...
                _onHLSJSLevelSwitched() {
                    ...
                    if (hls.levels.length > 1 && level && typeof level.bitrate === 'number') {
                        this._hls.config.abrEwmaDefaultEstimate = this.hlsConfig.abrEwmaDefaultEstimate = Math.round(level.bitrate * 0.8);
                ...
                _onHLSJSError(e, data) {
                    ...
                    console.info('%cHls.Events.ERROR: WARN', 'background: cyan;', this._id, data.type, 
                    // data,
                    ...
                    switch (data.type) {
                        ...
                        case Hls.ErrorTypes.MEDIA_ERROR:
                            if ([Hls.ErrorDetails.BUFFER_STALLED_ERROR, Hls.ErrorDetails.BUFFER_SEEK_OVER_HOLE].includes(data.details)) {
...
大きめの値を入れると最初から高画質が選択されやすくなる。
        ...
        const init = () => {
            ...
            const createVideoElement = (usecase) => {
                ...
                if (usecase === 'capture') {
                    ...
                    video.hlsConfig = Object.assign(hlsConfig, {
                        ...
                        abrEwmaDefaultEstimate: globalThis.ZenzaWatch.debug.hlsConfig.abrEwmaDefaultEstimate,
                        startLevel: lastLevel,
                    });
                    video.addEventListener('init-hls-js', (e) => {
                    ...
                    video.addEventListener('levelswitched', (e) => {
                        ...
                        if (Config.get('autoAbrEwmaDefaultEstimate')) {
                            globalThis.ZenzaWatch.debug.hlsConfig.abrEwmaDefaultEstimate = Math.round(detail.bitrate * 0.8);
                        }
                    });
                    primaryVideo = video;
                }
            ...
            void ZenzaDetector.detect('beforeready')
                ...
                .then((ZenzaWatch) => {
                void Promise.all([dimport('https://esm.run/lit@2.0.2/html.js')]).then((mods) => {
                    const { html, render } = mods[0];
