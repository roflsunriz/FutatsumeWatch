...
const workerUtil = (() => {
  ...
  `.trim();
    ...
    const getVideoTitle = (params = {}) => {
        ...
        const authorName = toSafeName(params.author || document.querySelector('#owner-container yt-formatted-string').textContent || '');
    ...
    const createCanvasFromVideo = (video) => {
        ...
        thumbnail.getContext('2d', { alpha: false, desynchronized: true }).drawImage(bitmap || canvas, 0, 0, thumbnail.width, thumbnail.height);
    ...
    const getThumbnailDataURL = async (width, height, type) => {
        ...
        canvas.width = dw;
        canvas.height = dh;
        canvas.getContext('2d', { alpha: false, desynchronized: true }).drawImage(video, 0, 0, dw, dh);
