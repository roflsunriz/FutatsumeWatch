...
void AntiPrototypeJs().then(() => {
    ...
    const monkey = (PRODUCT) => {
        ...
        //const $ = window.jQuery;
        console.log(`%c${PRODUCT}`, 'font-family: "Apple LiGothic"; padding: 4px; background: red; color: white; font-size: 150%;');
    ...
    `;
        ...
        const util = (MylistPocket.util = (() => {
            ...
            util.httpLink = function (html) {
                ...
                html = html.replace(/(https?:\/\/seiga\.nicovideo\.jp\/seiga\/)?im(\d+)/g, ' <a href="//seiga.nicovideo.jp/seiga/im$2" class="seigaLink" rel="noopener" target="_blank">$1im$2</a> ');
                html = html.replace(/(https?:\/\/com\.nicovideo\.jp\/community\/)?co(\d+)/g, ' <a href="//com.nicovideo.jp/community/co$2" class="communityLink" rel="noopener" target="_blank">$1co$2</a> ');
                html = html.replace(/(https?:\/\/www\.nicovideo\.jp\/)?(watch|shorts|mylist|series|user)\/(\d+)/g, ' <a href="https://www.nicovideo.jp/$2/$3" rel="noopener" class="videoLink target-change">$1$2/$3</a> ');
                html = html.replace(/(https?:\/\/www\.nicovideo\.jp\/watch\/)?(sm|nm|so|ss)(\d+)/g, ' <a href="https://www.nicovideo.jp/watch/$2$3" rel="noopener" class="videoLink target-change">$1$2$3</a> ');
                html = html.replace(/(https?:\/\/www\.nicovideo\.jp\/shorts\/)?ss(\d+)/g, ' <a href="https://www.nicovideo.jp/shorts/ss$2" rel="noopener" class="videoLink target-change">$1ss$2</a> ');
                const linkmatch = /<a.*?<\/a>/;
                let n;
...
const bounce = {
		...
		idle(func, time) {
				...
				const [caller, canceller] = (time === undefined && self.requestIdleCallback
...
const css = (() => {
		...
		const css = {
				addStyle: (styles, option, document = window.document) => {
						...
						elm.disabled = (typeof option === 'object' && option ? option.disabled : undefined);
				...
				number: (value) => (typedCSS.number ? typedCSS.number(value) : value),
				s: (value) => (typedCSS.s ? typedCSS.s(value) : `${value}s`),
				ms: (value) => (typedCSS.ms ? typedCSS.ms(value) : `${value}ms`),
				pt: (value) => (typedCSS.pt ? typedCSS.pt(value) : `${value}pt`),
				px: (value) => (typedCSS.px ? typedCSS.px(value) : `${value}px`),
				percent: (value) => (typedCSS.percent ? typedCSS.percent(value) : `${value}%`),
				vh: (value) => (typedCSS.vh ? typedCSS.vh(value) : `${value}vh`),
				vw: (value) => (typedCSS.vw ? typedCSS.vw(value) : `${value}vw`),
				trans: (value) => cssValueHost.CSSStyleValue ? cssValueHost.CSSStyleValue.parse('transform', value) : value,
				word: (value) => (cssValueHost.CSSKeywordValue ? new cssValueHost.CSSKeywordValue(value) : value),
				image: (value) => cssValueHost.CSSStyleValue ? cssValueHost.CSSStyleValue.parse('background-image', value) : value,
		};
		return css;
})();
const cssUtil = css;
...
const nicoUtil = {
		parseWatchQuery: (query) => {
				try {
						...
						const decoded = (typeof playlistSource === 'string' && playlistSource !== '' && textUtil.decodeBase64(playlistSource)) || '{}';
		...
		getCommonHeader: () => {
				try {
						const el = document.querySelector('#CommonHeader[data-common-header]');
						const parsed = JSON.parse(el.dataset.commonHeader || '{}');
						return parsed;
				}
				catch (e) {
						return { initConfig: {} };
		...
		isPremium: () => nicoUtil.isLegacyHeader() ? nicoUtil.isPremiumLegacy() : !!nicoUtil.getCommonHeader().initConfig.user.isPremium,
		isLogin: () => nicoUtil.isLegacyHeader() ? nicoUtil.isLoginLegacy() : !!nicoUtil.getCommonHeader().initConfig.user.isLogin,
		getPageLanguage: () => {
				try {
						const h = document.getElementsByClassName('html')[0];
						return h.lang || 'ja-JP';
				}
				catch (e) {
						return 'ja-JP';
		...
		openMylistWindow: (watchId) => {
				window.open(`//www.nicovideo.jp/mylist_add/video/${watchId}`, 'nicomylistadd', 'width=500, height=400, menubar=no, scrollbars=no');
		},
...
const textUtil = {
		...
		convertKansuEi: (text) => {
				...
				text = text.replace(/([1-9]?)[十拾]([0-9]?)/g, (n, a, b) => a && b ? `${a}${b}` : a ? String(a * 10) : String(10 + b * 1));
				return text;
		},
		dateToString: (date) => {
		...
		toRgba: (c, alpha = 1) => `rgba(${parseInt(c.substr(1, 2), 16)}, ${parseInt(c.substr(3, 2), 16)}, ${parseInt(c.substr(5, 2), 16)}, ${alpha})`,
...
const StorageWriter = (() => {
		...
		let worker;
		const prototypePollution = window.Prototype &&
...
class DataStorage {
		...
		clearConfig() {
				...
				Object.keys(this.default)
						...
						try {
								if (Object.prototype.hasOwnProperty.call(storage, storageKey) || loose[storageKey] !== undefined) {
										console.nicoru('delete storage', storageKey, loose[storageKey]);
										delete loose[storageKey];
								}
								this._data[key] = this.default[key];
						}
...
class CrossDomainGate extends Emitter {
		...
		videoCapture(src, sec) {
				return this._postMessage({ command: 'videoCapture', params: { src, sec } }).then((result) => Promise.resolve(result.dataUrl));
		}
		_fetch(url, options) {
		...
		async fetch(resource, options = {}) {
				...
				if (resource instanceof URL) {
						...
						options.signal ?? (options.signal = resource.signal);
				}
		...
		async configBridge(config) {
				...
				if (!this.constructor.hostReg.test(location.host) && !config.props.allowOtherDomain) {
						return;
				}
				config.on('update', (key, value) => {
        ...
        const ThumbInfoLoader = (() => {
            ...
            return {
                load: (watchId, options) => loader.load(watchId, options),
...
const MylistApiLoader = (() => {
		...
		class MylistApiLoader {
				...
				async _getCsrfToken() {
						...
						if (token) {
								...
								const result = await netUtil
										.fetch(tokenUrl, {
										cledentials: 'include',
								})
										.then((r) => r.text())
										.catch((err) => {
										throw new Error('マイリストトークン取得失敗', { result: err, status: 'fail' });
								});
								const dom = new DOMParser().parseFromString(result, 'text/html');
								const initUserpageDataContena = dom.querySelector('#js-initial-userpage-data');
								const envRaw = JSON.parse(String(initUserpageDataContena.getAttribute('data-environment')));
								const env = envRaw;
				...
				async _getDeflistItems(frontendId = 6, frontendVersion = 0) {
						...
						do {
								const raw = await netUtil
										...
										.then((r) => r.json())
										.catch((e) => {
										throw new Error('とりあえずマイリストの取得失敗(2)', e);
								});
								const res = raw;
				...
				async getMylistList({ frontendId = 6, frontendVersion = 0 } = {}) {
						...
						const raw = await netUtil
								...
								.then((r) => r.json())
								.catch((e) => {
								throw new Error('マイリスト一覧の取得失敗(2)', e);
						});
						const result = raw;
						if (result.meta.status !== 200 || !result.data.mylists) {
								throw new Error(`マイリスト一覧の取得失敗(1) ${String(result.status)}${String(result.message)}`, result);
						}
				...
				async removeDeflistItem(watchId, { frontendId = 6, frontendVersion = 0 } = {}) {
						const item = await this.findDeflistItemByWatchId(watchId).catch((err) => {
								throw new Error('動画が見つかりません', { result: err, status: 'fail' });
						});
				...
				async removeMylistItem(watchId, groupId, { frontendId = 6, frontendVersion = 0 } = {}) {
						const item = await this.findMylistItemByWatchId(watchId, groupId).catch((err) => {
								throw new Error('動画が見つかりません', { result: err, status: 'fail' });
						});
						...
						const raw = await netUtil
								...
								.then((r) => r.json())
								.catch((err) => {
								throw new Error('マイリストから削除失敗(2)', { result: err, status: 'fail' });
						});
						const result = raw;
						if (result.meta.status && result.meta.status === 200) {
								cacheStorage.removeItem(cacheKey);
        ...
        class HoverMenu extends Emitter {
            ...
            _init() {
                ...
                this._x = this._y = 0;
                void ZenzaDetector.detect().then((ZenzaWatch) => {
            ...
            _onHoverEnd(e) {
                ...
                this.show();
                this.moveTo(left + target.offsetWidth - this._view.offsetWidth / 2, top + target.offsetHeight / 2 - this._view.offsetHeight / 2);
            }
            _onMouseout(e) {
                const target = this._isTargetElement(e);
                if (!target) {
        ...
        class VideoInfoView extends Emitter {
            ...
            _initialize() {
                ...
                const updateNgEnable = (v) => {
                    this.toggleClass('is-ng-enable', v);
                };
                ...
                this._deflistButton = this._rootDom.querySelector('.mylistPocketButton.deflist-add');
                this.toggleClass('is-otherDomain', location.host !== 'www.nicovideo.jp');
            ...
            _onClick(e) {
                const t = e.target;
                const elm = t.classList.contains('command') ? t : e.target.closest('.command');
                if (!elm) {
                    return;
                }
                // 簡易 throttle
                ...
                switch (command) {
                    ...
                    case 'toggle-fav-owner':
                        {
                            const owner = (this._videoInfo.isChannel ? 'ch' : '') + this._videoInfo.ownerId + '#' + this._videoInfo.ownerName;
                            this.emit('command', command, {
                                watchId: this._videoInfo.watchId,
                                value: owner,
                            }, this);
            ...
            _updateFavNg() {
                ...
                ownerInfo.classList.toggle('is-favorited', this._favChecker.isMatchOwner(videoInfo.owner));
                ownerInfo.classList.toggle('is-ng', this._ngChecker.isMatchOwner(videoInfo.owner));
                Array.prototype.forEach.call(this._rootDom.querySelectorAll('.tag-container'), (elm) => {
                    const tag = elm.getAttribute('data-tag');
            ...
            _bindSuccess(videoInfo) {
                ...
                Array.prototype.forEach.call(this._host.querySelectorAll('.target-change'), (elm) => {
                    elm.target = target;
                    elm.rel = 'noopener';
                });
                this._updateFavNg();
        ...
        class VideoInfo {
            static createByThumbInfo(thumbInfo) {
                ...
                if (util.hasLargeThumbnail(thumbInfo.videoId)) {
                    thumbnail = thumbnail.replace(/\.[ML]$/, undefined) + '.L';
                }
                ...
                const rawData = {
                    ...
                    tags: thumbInfo.tagList.map((tag) => {
                        return { text: tag.text, isLocked: tag.lock };
                    }),
                };
        ...
        const createCommandDispatcher = ({ infoView }) => {
            ...
            const toggleFavNg = (command, param) => {
                ...
                Array.prototype.forEach.call(document.querySelectorAll(`*[data-watch-id=${param.watchId}]`), (item) => {
                ...
                switch (command) {
                    ...
                    case 'fav-status':
                        return QueueLoader.load(param).then((result) => {
                            ...
                            if (ngChecker.isMatch(res)) {
                                return { status: 'ng', result: res };
                            }
                            if (favChecker.isMatch(res)) {
                                return { status: 'favorite', result: res };
                            }
                            return { status: 'default', result: res };
                        });
                    case 'mylist-window':
                        window.open(protocol + '//www.nicovideo.jp/mylist_add/video/' + param, 'nicomylistadd', 'width=500, height=400, menubar=no, scrollbars=no');
                        break;
            ...
            const onItemInview = (item) => {
                ...
                if (!watchId) {
                    const a = (item instanceof HTMLAnchorElement
                        ? item
                        : item.querySelector('a[href*=\'watch/\'],a[href*="shorts/"]'));
                    let m;
                    if (a != null &&
        ...
        const initNg = async (params) => {
            ...
            const onItemInview = (item, watchId) => {
                ...
                QueueLoader.load(watchId, item).then((info) => {
                    ...
                    if (!info || info.status === 'fail' || info.code === 'DELETED') {
                        ...
                        if (callback) {
                            return callback(item, {
                                watchId,
                                info,
                                isNg: ngChecker.isNg(info),
                                isFav: favChecker.isMatch(info),
                            });
                        }
                        item.classList.add(ngChecker.isNg(info) ? 'is-ng-rejected' : 'is-ng-resolved');
                        if (favChecker.isMatch(info)) {
                            item.classList.add('is-fav-favorited');
                        }
                        for (const img of item.querySelectorAll('img.videoThumbnail.preview')) {
                        ...
                        for (const a of item.querySelectorAll(`a[href*="shorts/${watchId}"]`)) {
                            const href = a.getAttribute('href');
                            a.setAttribute('href', href.replace(/shorts\/([0-9]+)/, `shorts/${info.id}`).replace(/^http:/, 'https:'));
                        }
                    }
                    loadLazy();
                }, () => {
        ...
        const init = async () => {
            ...
            hoverMenu.on('info', ((watchId) => {
                ...
                void dispatcher('info', watchId).then(() => {
...
const workerUtil = (() => {
						...
						try {
										...
										case 'port':
												{
														...
														port.addEventListener('message', (ev) => {
																void onMessage(port, params.name, ev);
														});
										...
										case 'broadcast':
												{
														...
														channel.addEventListener('message', (ev) => {
																void onMessage(channel, 'BroadcastChannel', ev);
														});
								...
								self.postMessage({
										body: {
												...
												params: { command, result: err.message || null },
										},
										sessionId,
										TYPE: type,
										PID,
		...
		const workerUtil = {
				...
				env: (params) => {
						({ config, TOKEN, PRODUCT, netUtil, CONSTANT, global } = Object.assign({ config, TOKEN, PRODUCT, netUtil, CONSTANT, global }, params));
						if (global) {
								...
								try {
										...
										switch (command) {
												...
												case 'emit':
														...
														break;
												case 'fetch':
														result = await (netUtil || window).fetch(params.url, Object.assign({}, params.options || {}, { _format: 'arraybuffer' }));
...
const IndexedDbStorage = (() => {
		...
		const open = async ({ name, ver, stores }, func
				...
				`;
						...
						worker =
								workers.get(func) ||
										workerUtil.createCrossMessageWorker(_func, { name: `IndexedDb[${name}]` });
						workers.set(func, worker);
				}
				else {
						worker =
								workers.get(workerFunc) ||
										workerUtil.createCrossMessageWorker(workerFunc, { name: 'IndexedDb' });
...
const gate = () => {
		...
		const isWhiteHost = (url) => {
				...
				const whiteHost = localStorage.FutatsumeWatch_whiteHost ??
						localStorage.ZenzaWatch_whiteHost;
				if (whiteHost) {
		...
		const xFetch = (params, sessionId = null) => {
				...
				return uFetch(params)
						...
						const init = ['type', 'url', 'redirected', 'status', 'ok', 'statusText'].reduce((map, key) => {
								map[key] = r[key];
								return map;
						}, {});
