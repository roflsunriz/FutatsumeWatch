...
// @noframes
// @downloadURL    https://github.com/roflsunriz/FutatsumeWatch/raw/main/dist/MylistFilter.user.js
// ==/UserScript==
/* eslint-disable */
void (async (window) => {
    const global = {
...
const dimport = (() => {
		try {
				const factory = new Function('u', 'return import(u)');
				return factory;
		}
		catch {
				const map = {};
...
const cssUtil = css;
    const [lit] = await Promise.all([dimport('https://esm.run/lit')]);
    const { html } = lit;
    const $ = self.jQuery;
    cssUtil.addStyle(`
  ...
  `);
    ...
    const resetForm = () => {
        [...document.querySelectorAll('.itemFilterContainer input[name="playable-filter"]')].forEach((r) => (r.checked = r.hasAttribute('checked')));
        [...document.querySelectorAll('.wordFilterInput')].forEach((r) => (r.value = r.getAttribute('value')));
    };
    ...
    const overrideFilter = () => {
        ...
        Object.defineProperty(self, 'wordFilterCallback', {
            get: () => {
                ...
                return word
                    ? (item) => {
                        return ((item.item_data.title || '').toLowerCase().indexOf(word) >= 0 ||
    ...
    const parseProps = () => {
        if (!location.hash || location.length <= 2) {
            return {};
        }
