        ...
        Object.keys(DEFAULT_CONFIG)
            ...
            def[key] = {
                ...
                set(value) {
                    ...
                    document.body.dispatchEvent(new CustomEvent(`${PRODUCT}-config.update`, {
...
const cssUtil = css;
        const 業務 = function (self) {
            ...
            const detect = async ({ bitmap, }) => {
                ...
                const tasks = [];
                if (faceDetection) {
                    tasks.push(faceDetector.detect(bitmap).catch(() => []));
                }
                if (textDetection) {
                    tasks.push(textDetector.detect(bitmap).catch(() => []));
                }
                ...
                paint(ctx, { width, height }, props) {
                    ...
                    for (const boxes of history) {
                        ...
                        for (const box of boxes) {
                            ...
                            let x = boxX, y = boxY, width = boxWidth, height = boxHeight;
                            const type = boxType;
        ...
        const createDetector = async ({ video, layer, interval, type, }) => {
            ...
            let isBusy = true, currentTime = video.currentTime;
            const boxHistory = [];
        ...
        const ZenzaDetector = (() => {
            ...
            const promise = zenzaWindow.ZenzaWatch && zenzaWindow.ZenzaWatch.ready
                ? Promise.resolve(zenzaWindow.ZenzaWatch)
                : new Promise((resolve) => {
                    [window, document.body || document.documentElement].forEach((e) => e.addEventListener('ZenzaWatchInitialize', () => {
        ...
        const watch = () => {
            ...
            [...document.querySelectorAll('video, zenza-video')]
                ...
                if (layer) {
                    video.dispatchEvent(new CustomEvent(`${PRODUCT}-start`, { detail: { type, video, layer }, bubbles: true, composed: true }));
                }
                vmap.set(video, layer
                    ? createDetector({
                        video: (video.drawableElement ||
                            video),
                        layer: layer,
        ...
        const init = () => {
            css.registerProps({ name: '--json-args', syntax: '*', initialValue: '{}', inherits: false }, { name: '--config', syntax: '*', initialValue: '{}', inherits: false });
