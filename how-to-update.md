...
UIだけを確認する場合は`bun run test:browser ui`を実行します。配布物を注入し、中央操作、設定初期画面の左レール／右選択案内、平置きした3操作、一般・詳細設定の導線、ABリピート、狭幅・低高さ・4Kを確認します。画像と結果は`dev-assets/verification/shell-*`へ保存します。コメント投稿フォームも、音量バー横の配置、パレット、実入力、Enter・投稿ボタン、失敗時の本文保持と再送、全画面・各寸法を確認します。送信結果は専用タブ内で制御し、公開コメント投稿やタグ編集の送信は行いません。

設定パネルは`bun run test:browser settings`で、一般設定と詳細設定の背景クリック・Escape・閉じるボタン、左カテゴリ／右項目の全画面表示、36項目の入力と保存・再表示、390px以下の縦配置、低高さを確認します。保存先は`dev-assets/verification/settings-*`です。設定値は自動テスト用プロファイル内だけで変更し、確認後に元の値へ戻します。
