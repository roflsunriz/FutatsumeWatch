...
| P2-07 | 一般プレイヤー設定の全入力、再生・クリック・全画面・提供者表示への接続 | settings、settings-autoplay／settings-playback-effects／settings-sponsors／settings-library-effects単体 |
...
| クリック・ダブルクリック全画面・自動全画面・タッチ有効化／取消・提供者表示 | `verify-settings-player`の実入力と描画、`settings-sponsors`の遅延応答                                                          |
