...
interface InitializerSteps {
  ...
  overrideGinza: (dialog: InitializerDialog, query: GinzaSlayerQuery) => Promise<void>;
}

const { initialize, initializeExternal, initializeMessage, initializeLastSession, overrideGinza } =
  ((): InitializerSteps => {
    //@require hover-menu
    // GINZAを置き換えるべきか？の判定
    ...
    const initializeMessage = (player: InitializerDialog): void => {
      ...
      const onBroadcastMessage = (cmd: InitializerCommandBody, type: string, sessionId: string): void => {
        ...
        if (command === 'hello') {
          ...
          (WatchPageHistory as unknown as InitializerWatchPageHistory).pushHistoryAgency(
            path as string,
            title as string
          );
    ...
    return { initialize, initializeExternal, initializeMessage, initializeLastSession, overrideGinza };
  })();

//===END===

export { initialize, initializeExternal, initializeMessage, initializeLastSession, overrideGinza };
