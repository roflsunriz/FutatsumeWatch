...
- 大百科アイコンはwatch応答の真偽値を表示へ直結せず、全タグを`api.dic.nicovideo.jp/v1/articles/article/<タグ>`へ照会する。2xxだけを「あり」、404だけを「なし」、それ以外を「不明」とし、API本文の世代差を存在判定へ持ち込まない。回帰は`tag-dictionary.test.ts`とlibraryスイート。
- 設定UIは2026-09-22の手描き案を正本とする。入口は画面全体を左レールと右の「設定項目を選んでください」に分け、左に製品名・版・カテゴリ入口・画質・GitHub・3つの操作を置く。「その他操作」のdetailsは使わない。カテゴリ選択後も画面全体の左カテゴリ／右項目を維持し、狭幅だけ縦配置へ切り替える。回帰はui/settingsスイートと複数画面寸法の画像。
- 動画詳細UIは2026-09-22の手描きUI案2を正本とする。4タブ共通の固定ボタンをタブ列へ置き、固定中はモーダルの背景ぼかし・操作抑止を外し、映像・コメント面・操作バーを右パネル幅ぶん縮めて全体を見せる。タグは詳細内の閲覧専用表示とし、編集・追加・削除・再取得ボタンやタグ書き込みAPIを復活させない。回帰はui/libraryスイート。
...
- 設定画面は利用可能な表示領域全体を使い、左カテゴリと右本文を別々にスクロールする。390px以下はカテゴリを上、本文を下へ並べる。Shadow DOM内と通常DOM内のパネルで外枠寸法をそろえるため、共通テーマでbox-sizingを明示する。
