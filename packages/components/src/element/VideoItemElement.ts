...
const { VideoItemElement, VideoItemProps } = (() => {
  ...
  class VideoItemElement extends BaseCommandElement {
    ...
    _applyThumbInfo(thumbInfo: ThumbInfoPayload): void {
      const data = (thumbInfo.data || thumbInfo) as ThumbData; // legacy 互換のため
      const currentThumbnail = this.props.thumbnail as string;
      const thumbnail = currentThumbnail.match(/smile\?i=/) ? currentThumbnail : data.thumbnail;
      ...
      this.dispatchEvent(
        new CustomEvent('thumb-info', { detail: { props: this.props }, bubbles: true, composed: true })
      );
    }
    ...
    attributeChangedCallback(attr: string, oldValue: string | null, newValue: string | null): void {
      ...
      const info = JSON.parse(newValue as string) as ThumbInfoPayload;
