...
import type { CommandDetail, ElementEvents, LitModule, PropsMap, StateMap } from './base-command-element.js';
...
const { VideoSeriesProps, VideoSeriesLabel } = (() => {
  ...
  class VideoSeriesLabel extends BaseCommandElement {
    ...
    onCommand(e: Event): void {
      const { command, param } = (e as CustomEvent<CommandDetail>).detail;
      if (command === 'open-window') {
        window.open(param as string);
      }
    }
  }
