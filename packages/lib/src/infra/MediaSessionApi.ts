...
import type { EmitterCallback } from '../Emitter';
...
const MediaSessionApi = (() => {
  ...
  const update = (
    { title, artist, album, artwork, duration }: MediaSessionMetadata = {
      title: '',
      artist: '',
      album: '',
      artwork: [],
    ...
    nm.setActionHandler('seekto', (e) => {
      emitter.emit('command', 'seekTo', (e as { seekTime?: unknown }).seekTime);
    });
  };

  const updateByVideoInfo = (videoInfo: VideoInfoLike): void => {
    const title = videoInfo.title;
    const artist = videoInfo.owner.name;
    const album = '';
    const artwork: { src: string; sizes: string; type: string }[] = [
      { src: videoInfo.thumbnail, sizes: '130x100', type: 'image/jpg' },
    ];
  ...
  const updatePositionState = ({
    ...
    nm.setPositionState({ duration, playbackRate, currentTime } as unknown as MediaPositionState);
  };
  const updatePositionStateByMedia = (media: HTMLMediaElement): void => {
    updatePositionState({
  ...
  return {
    onCommand: (callback: EmitterCallback) => emitter.on('command', callback),
    update,
    updateByVideoInfo,
    updatePositionState,
