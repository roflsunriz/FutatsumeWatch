type IsObject = (e: unknown) => e is Record<string | symbol, unknown>;
...
interface ObjUtil {
  bridge: (self: BridgeTarget, target: BridgeTarget, keys?: string[] | null) => void;
  isObject: IsObject;
  toMap: (obj: unknown, mapper?: typeof Map) => Map<unknown, unknown>;
  ...
  return {
    bridge: (self, target, keys = null) => {
      (keys || Object.getOwnPropertyNames(target.constructor.prototype))
        ...
        .forEach((key) => (self[key] = (target[key] as (...args: unknown[]) => unknown).bind(target)));
    },
    isObject,
    toMap: (obj, mapper = Map) => {
      if (obj instanceof mapper) {
        return obj as Map<unknown, unknown>;
      }
      return new mapper(Object.entries(obj as Record<string, unknown>));
    },
    mapToObj: (map) => {
