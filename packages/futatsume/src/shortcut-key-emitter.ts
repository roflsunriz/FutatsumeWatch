...
interface ShortcutExternalEmitter {
  promise(name: string): Promise<unknown>;
  on(name: string, handler: (e: ShortcutKeyEvent) => void): void;
}

interface ShortcutEmitter {
  emit(eventName: string, ...args: unknown[]): void;
}

// Chrome 系の非標準 Event.path を含むキーイベント。
...
class ShortcutKeyEmitter {
  static create(
    ...
    externalEmitter: ShortcutExternalEmitter
  ): ShortcutEmitter {
    const emitter = new (Emitter as unknown as new () => ShortcutEmitter)();
    ...
    Object.keys(map).forEach((key) => {
      map[key] = parseInt(config.props[`KEY_${key}`] as string, 10);
    });

    const onKeyDown = (e: ShortcutKeyEvent): void => {
      const target = (e.path && e.path[0] ? e.path[0] : e.target) as unknown as { tagName?: string };
    ...
    const onKeyUp = (e: ShortcutKeyEvent): void => {
      const target = (e.path && e.path[0] ? e.path[0] : e.target) as unknown as { tagName?: string };
      if (target.tagName === 'SELECT' || target.tagName === 'INPUT' || target.tagName === 'TEXTAREA') {
        return;
      }
