...
class VideoListItemView {
  declare static map: WeakMap<VideoListItem, ItemViewCache>;
  static get ITEM_HEIGHT(): number {
    return 100;
  }
  static get THUMBNAIL_WIDTH(): number {
    return 96;
  }
  ...
  * {
  ...
  static build(item: VideoListItem, index = 0): unknown {
    const dllLike = dll as unknown as DllLike;
    const textUtilLike = textUtil as unknown as TextUtilLike;

    const addComma = (m: unknown): string =>
      isNaN(m as number) ? '---' : String((m as { toLocaleString?: () => unknown }).toLocaleString?.() ?? m);
    const {
      cache,
      timestamp,
      index: _index,
    } = this.map.get(item) ?? ({} as { cache?: unknown; timestamp?: number; index?: number });
    ...
    const thumbnailStyle = `background-image: url(${String(item.thumbnail)})`;

    const result = dllLike.lit.html` <div
      class=${dllLike.directives.classMap(cmap)}
      data-index=${index + 1}
      data-item-id=${itemId}
    ...
    >
      ${
        item.isLazy
          ...
          : dllLike.lit.html`
              <span
                class="command playlistRemove"
                data-command="playlistRemove"
                data-param=${watchId}
                title="プレイリストから削除"
                >×</span
              >
              ...
              >
                <a class="command" href=${watchUrl} data-command="select" data-param=${itemId}>
                  <span class="duration">${textUtilLike.secToTime(item.duration)}</span>
                </a>
                <span
                  class="command playlistAppend"
