import { Emitter } from '../../../lib/src/emitter';
import { textUtil } from '../../../lib/src/text/text-util';
import type { NicoChatType as NicoChat } from './nico-chat';

interface ParsedScript {
  id: number;
  type: string;
  params: unknown;
}

interface NicoScriptReplaceParams {
  src: string;
  dest: string;
  fill: boolean;
  target: string;
  partial: boolean;
}

interface NicoScriptSeekParams {
  time: unknown;
}

interface NicoScriptReverseParams {
  target: string;
}

interface NicoScriptJumpParams {
  target: string;
  type: string;
  time: number | string;
}

interface NicoScriptMarkerParams {
  name: unknown;
}

interface NicoScripterGroup {
  members?: NicoChat[];
}

interface TextUtilLike {
  escapeRegs(value: string): string;
}

export type {
  ParsedScript,
  NicoScriptReplaceParams,
  NicoScriptSeekParams,
  NicoScriptReverseParams,
  NicoScriptJumpParams,
  NicoScriptMarkerParams,
};

//===BEGIN===

class NicoScriptParser {
  declare static _count: number;
  static get parseId(): number {
    if (!NicoScriptParser._count) {
      NicoScriptParser._count = 1;
    }
    return NicoScriptParser._count++;
  }

  static parseNiwango(lines: string[]): ParsedScript[] {
    // 構文はいったん無視して、対応できる命令だけ拾っていく。
    // ニワン語のフル実装は夢
    let type: string, params: unknown, m: RegExpExecArray | null;
    const result: ParsedScript[] = [];
    for (let i = 0, len = lines.length; i < len; i++) {
      const text = lines[i] as string;
      const id = NicoScriptParser.parseId;
      if ((m = /^\/?replace\((.*?)\)/.exec(text)) !== null) {
        type = 'REPLACE';
        params = NicoScriptParser.parseReplace(m[1] as string);
        result.push({ id, type, params });
      } else if ((m = /^\/?commentColor\s*=\s*0x([0-9a-f]{6})/i.exec(text)) !== null) {
        result.push({ id, type: 'COLOR', params: { color: '#' + (m[1] as string) } });
      } else if ((m = /^\/?seek\((.*?)\)/i.exec(text)) !== null) {
        params = NicoScriptParser.parseSeek(m[1] as string);
        result.push({ id, type: 'SEEK', params });
      }
    }
    return result;
  }

  static parseParams(str: string): Record<string, string> | null {
    // 雑なパース
    const result: Record<string, string> = {};
    let v = '',
      lastC = '',
      key: string | undefined,
      isStr = false,
      quot = '';
    for (let i = 0, len = str.length; i < len; i++) {
      const c = str.charAt(i);
      switch (c) {
        case ':':
          key = v.trim();
          v = '';
          break;
        case ',':
          if (isStr) {
            v += c;
          } else {
            if (key !== '' && v !== '') {
              result[key as string] = v.trim();
            }
            key = v = '';
          }
          break;
        case ' ':
          if (v !== '') {
            v += c;
          }
          break;
        case "'":
        case '"':
          if (v !== '') {
            if (quot !== c) {
              v += c;
            } else if (isStr) {
              if (lastC === '\\') {
                v += c;
              } else {
                if (quot === '"') {
                  // ダブルクォートの時だけエスケープがあるらしい
                  v = v.replace(/(\\r|\\n)/g, '\n').replace(/(\\t)/g, '\t');
                }
                result[key as string] = v;
                key = v = '';
                isStr = false;
              }
            } else {
              window.console.error('parse fail?', isStr, lastC, str);
              return null;
            }
          } else {
            quot = c;
            isStr = true;
          }
          break;
        default:
          v += c;
      }
      lastC = c;
    }
    if (key !== '' && v !== '') {
      result[key as string] = v.trim();
    }

    return result;
  }

  static parseNicosParams(str: string): string[] {
    // 雑なパース
    const result: string[] = [];
    let v = '',
      lastC = '',
      quot = '';
    for (let i = 0, len = str.length; i < len; i++) {
      const c = str.charAt(i);
      switch (c) {
        case ' ':
        case '　':
          if (quot) {
            v += c;
          } else {
            if (v !== '') {
              result.push(v);
              v = quot = '';
            }
          }
          break;
        case "'":
        case '"':
          if (v !== '') {
            if (quot !== c) {
              v += c;
            } else {
              if (lastC === '\\') {
                v += c;
              } else {
                v = v.replace(/(\\r|\\n)/g, '\n').replace(/(\\t)/g, '\t');
                result.push(v);
                v = quot = '';
              }
            }
          } else {
            quot = c;
          }
          break;
        case '「':
          if (v !== '') {
            v += c;
          } else {
            quot = c;
          }
          break;
        case '」':
          if (v !== '') {
            if (quot !== '「') {
              v += c;
            } else {
              if (lastC === '\\') {
                v += c;
              } else {
                result.push(v);
                v = quot = '';
              }
            }
          } else {
            v += c;
          }
          break;
        default:
          v += c;
      }
      lastC = c;
    }
    if (v !== '') {
      result.push(v.trim());
    }

    return result;
  }

  static parseNicos(text: string): ParsedScript {
    text = text.trim();
    const text1 = (text || '').split(/[ 　:：]+/)[0] ?? ''; // eslint-disable-line
    let params: unknown;
    let type: string;
    switch (text1) {
      case '@デフォルト':
      case '＠デフォルト':
        type = 'DEFAULT';
        break;
      case '@逆':
      case '＠逆':
        type = 'REVERSE';
        params = NicoScriptParser.parse逆(text);
        break;
      case '@ジャンプ':
      case '＠ジャンプ': {
        const jump = NicoScriptParser.parseジャンプ(text);
        params = jump;
        type = jump.type;
        break;
      }
      case '@ジャンプマーカー':
      case '＠ジャンプマーカー':
        type = 'MARKER'; //@ジャンプマーカー：ループ
        params = NicoScriptParser.parseジャンプマーカー(text);
        break;
      default:
        if (text.indexOf('@置換') === 0 || text.indexOf('＠置換') === 0) {
          type = 'REPLACE';
          params = NicoScriptParser.parse置換(text);
        } else {
          type = 'PIPE';
          const lines = NicoScriptParser.splitLines(text);
          params = NicoScriptParser.parseNiwango(lines);
        }
    }

    const id = NicoScriptParser.parseId;
    return { id, type, params };
  }

  static splitLines(str: string): string[] {
    const result: string[] = [];
    let v = '',
      lastC = '',
      isStr = false,
      quot = '';
    for (let i = 0, len = str.length; i < len; i++) {
      const c = str.charAt(i);
      switch (c) {
        case ';':
          if (isStr) {
            v += c;
          } else {
            result.push(v.trim());
            v = '';
          }
          break;
        case ' ':
          if (v !== '') {
            v += c;
          }
          break;
        case "'":
        case '"':
          if (isStr) {
            if (quot === c) {
              if (lastC !== '\\') {
                isStr = false;
              }
            }
            v += c;
          } else {
            quot = c;
            isStr = true;
            v += c;
          }
          break;
        default:
          v += c;
      }
      lastC = c;
    }
    if (v !== '') {
      result.push(v.trim());
    }

    return result;
  }

  static parseReplace(str: string): NicoScriptReplaceParams | null {
    const result = NicoScriptParser.parseParams(str);

    if (!result) {
      return null;
    }
    return {
      src: result.src as string,
      dest: result.dest || '',
      fill: result.fill === 'true' ? true : false,
      target: result.target || 'user',
      partial: result.partial === 'false' ? false : true,
    };
  }

  static parseSeek(str: string): NicoScriptSeekParams | null {
    const result = NicoScriptParser.parseParams(str);
    if (!result) {
      return null;
    }
    return {
      time: result.vpos,
    };
  }

  static parse置換(str: string): NicoScriptReplaceParams {
    const tmp = NicoScriptParser.parseNicosParams(str);
    //＠置換 キーワード 置換後 置換範囲 投コメ 一致条件
    //＠置換 "И"       "██" 単       投コメ

    // 投稿者コメントを含めるかどうか
    let target = 'user'; // '投コメ'
    if (tmp[4] === '含む' || tmp[4] === '全') {
      // マニュアルにはないが '全' もあるらしい
      target = 'owner user';
    } else if (tmp[4] === '投コメ') {
      target = 'owner';
    }
    return {
      src: tmp[1] as string,
      dest: tmp[2] || '',
      fill: tmp[3] === '全' ? true : false, //全体を置き換えるかどうか
      target, //(tmp[4] === '含む' || tmp[4] === '投コメ')     ? 'owner user' : 'user',
      partial: tmp[5] === '完全一致' ? false : true, // 完全一致のみを見るかどうか
    };
  }

  static parse逆(str: string): NicoScriptReverseParams {
    const tmp = NicoScriptParser.parseNicosParams(str);
    /* eslint-disable */
    //＠逆　投コメ
    /* eslint-enable */
    const target = (tmp[1] || '').trim();
    //＠置換キーワード置換後置換範囲投コメ一致条件
    return {
      target: target === 'コメ' || target === '投コメ' ? target : '全',
    };
  }

  static parseジャンプ(str: string): NicoScriptJumpParams {
    //＠ジャンプ ジャンプ先 メッセージ 再生開始位置 戻り秒数 戻りメッセージ
    const tmp = NicoScriptParser.parseNicosParams(str);
    const target = tmp[1] || '';
    let type = 'JUMP';
    let time: number | string = 0;
    let m: RegExpExecArray | null;
    if ((m = /^#(\d+):(\d+)$/.exec(target)) !== null) {
      type = 'SEEK';
      time = Number(m[1]) * 60 + Number(m[2]) * 1;
    } else if ((m = /^#(\d+):(\d+\.\d+)$/.exec(target)) !== null) {
      type = 'SEEK';
      time = Number(m[1]) * 60 + Number(m[2]) * 1;
    } else if ((m = /^(#|＃)(.+)/.exec(target)) !== null) {
      type = 'SEEK_MARKER';
      time = m[2] as string;
    }
    return { target, type, time };
  }

  static parseジャンプマーカー(str: string): NicoScriptMarkerParams {
    const tmp = NicoScriptParser.parseNicosParams(str);
    const name = (tmp[0] as string).split(/[:： 　]/)[1]; // eslint-disable-line
    return { name };
  }
}

class NicoScripter extends Emitter {
  declare _hasSort: boolean;
  declare _list: NicoChat[];
  declare _eventScript: Array<{ p: ParsedScript; nicos: NicoChat }>;
  declare _nextVideo: string | null;
  declare _marker: Record<string, number>;
  declare _inviewEvents: Record<number, boolean>;
  declare _currentTime: number;
  declare _eventId: number;
  constructor() {
    super();
    this.reset();
  }

  reset(): void {
    this._hasSort = false;
    this._list = [];
    this._eventScript = [];
    this._nextVideo = null;
    this._marker = {};
    this._inviewEvents = {};
    this._currentTime = 0;
    this._eventId = 0;
  }

  add(nicoChat: NicoChat): void {
    this._hasSort = false;
    this._list.push(nicoChat);
  }

  get isEmpty(): boolean {
    return this._list.length === 0;
  }

  getNextVideo(): string {
    return this._nextVideo || '';
  }

  getEventScript(): Array<{ p: ParsedScript; nicos: NicoChat }> {
    return this._eventScript || [];
  }

  get currentTime(): number {
    return this._currentTime;
  }

  set currentTime(v: number) {
    this._currentTime = v;
    if (this._eventScript.length > 0) {
      this._updateInviewEvents();
    }
  }

  _sort(): void {
    if (this._hasSort) {
      return;
    }
    const list = this._list.concat().sort((a, b) => {
      const av = a.vpos,
        bv = b.vpos;
      if (av !== bv) {
        return av - bv;
      } else {
        return a.no < b.no ? -1 : 1;
      }
    });
    this._list = list;
    this._hasSort = true;
  }

  _updateInviewEvents(): void {
    const ct = this._currentTime;
    this._eventScript.forEach(({ p, nicos }) => {
      const beginTime = nicos.vpos / 100;
      const endTime = beginTime + nicos.duration;
      if (beginTime > ct || endTime < ct) {
        delete this._inviewEvents[p.id];
        return;
      }
      if (this._inviewEvents[p.id]) {
        return;
      }
      this._inviewEvents[p.id] = true;
      let diff = nicos.vpos / 100 - ct;
      diff = Math.min(1, Math.abs(diff)) * (diff / Math.abs(diff));
      switch (p.type) {
        case 'SEEK':
          this.emit('command', 'nicosSeek', Math.max(0, Number((p.params as NicoScriptSeekParams).time) + diff));
          break;
        case 'SEEK_MARKER': {
          const markerTime = (p.params as NicoScriptJumpParams).time;
          const time = this._marker[String(markerTime)] || 0;
          this.emit('command', 'nicosSeek', Math.max(0, time + diff));
          break;
        }
      }
    });
  }

  apply(group: NicoScripterGroup | NicoChat[]): void {
    this._sort();
    const assigned: Record<number, boolean> = {};

    // どうせ全動画の1%も使われていないので
    // 最適化もへったくれもない
    const eventFunc: Record<string, (p: ParsedScript, nicos: NicoChat) => void> = {
      JUMP: (p) => {
        const target = String((p.params as NicoScriptJumpParams).target);
        if (/^([a-z]{2}|)[0-9]+$/.test(target)) {
          this._nextVideo = target;
        }
      },
      SEEK: (p, nicos) => {
        if (assigned[p.id]) {
          return;
        }
        assigned[p.id] = true;
        this._eventScript.push({ p, nicos });
      },
      SEEK_MARKER: (p, nicos) => {
        if (assigned[p.id]) {
          return;
        }
        assigned[p.id] = true;

        this._eventScript.push({ p, nicos });
      },
      MARKER: (p, nicos) => {
        this._marker[String((p.params as NicoScriptMarkerParams).name)] = nicos.vpos / 100;
      },
    };

    const applyFunc: Record<string, (nicoChat: NicoChat, nicos: NicoChat, params: unknown) => void> = {
      DEFAULT(nicoChat, nicos) {
        const nicosColor = nicos.color;
        const hasColor = nicoChat.hasColorCommand;
        if (nicosColor && !hasColor) {
          nicoChat.color = nicosColor;
        }

        const nicosSize = nicos.size;
        const hasSize = nicoChat.hasSizeCommand;
        if (nicosSize && !hasSize) {
          nicoChat.size = nicosSize;
        }

        const nicosType = nicos.type;
        const hasType = nicoChat.hasTypeCommand;
        if (nicosType && !hasType) {
          nicoChat.type = nicosType;
        }
      },
      COLOR(nicoChat, nicos, params) {
        const hasColor = nicoChat.hasColorCommand;
        if (!hasColor) {
          nicoChat.color = (params as { color: string }).color;
        }
      },
      REVERSE(nicoChat, nicos, params) {
        const target = (params as NicoScriptReverseParams).target;
        if (target === '全') {
          nicoChat.isReverse = true;
        } else if (target === '投コメ') {
          if (nicoChat.fork > 0) {
            nicoChat.isReverse = true;
          }
        } else if (target === 'コメ') {
          if (nicoChat.fork === 0) {
            nicoChat.isReverse = true;
          }
        }
      },
      REPLACE(nicoChat, nicos, params) {
        const replaceParams = params as NicoScriptReplaceParams;
        if (!replaceParams) {
          return;
        }
        // if (nicoChat.isNicoScript()) { return; }
        if (nicoChat.fork > 0 && (replaceParams.target || '').indexOf('owner') < 0) {
          return;
        }
        if (nicoChat.fork < 1 && replaceParams.target === 'owner') {
          return;
        }

        let isMatch: boolean;
        let text = nicoChat.text;

        if (replaceParams.partial === true) {
          isMatch = text.indexOf(replaceParams.src) >= 0;
        } else {
          isMatch = text === replaceParams.src;
        }
        if (!isMatch) {
          return;
        }

        if (replaceParams.fill === true) {
          text = replaceParams.dest;
        } else {
          // ＠置換 "~" "\n" 単 全
          const reg = new RegExp((textUtil as unknown as TextUtilLike).escapeRegs(replaceParams.src), 'g');
          text = text.replace(reg, replaceParams.dest);
        }
        nicoChat.text = text;

        // 注意: 原本のまま `clor` を参照する（`color` の Typo と思われるが
        // 実行時の挙動を変えないため修正しない。別タスクで上流確認のこと）。
        const nicosColor = (nicos as unknown as { clor?: string | null }).clor;
        const hasColor = nicoChat.hasColorCommand;
        if (nicosColor && !hasColor) {
          nicoChat.color = nicosColor;
        }

        const nicosSize = nicos.size;
        const hasSize = nicoChat.hasSizeCommand;
        if (nicosSize && !hasSize) {
          nicoChat.size = nicosSize;
        }

        const nicosType = nicos.type;
        const hasType = nicoChat.hasTypeCommand;
        if (nicosType && !hasType) {
          nicoChat.type = nicosType;
        }
      },
      PIPE(nicoChat, nicos, params) {
        const lines = params as ParsedScript[];
        lines.forEach((line) => {
          const type = line.type;
          const f = applyFunc[type];
          if (f) {
            f(nicoChat, nicos, line.params);
          }
        });
      },
    };

    this._list.forEach((nicos) => {
      const p = NicoScriptParser.parseNicos(nicos.text);
      if (!p) {
        return;
      }
      if (!nicos.hasDurationSet) {
        nicos.duration = 99999;
      }

      const ev = eventFunc[p.type];
      if (ev) {
        return ev(p, nicos);
      } else if (p.type === 'PIPE') {
        for (const line of p.params as ParsedScript[]) {
          const type = line.type;
          const ev = eventFunc[type];
          if (ev) {
            ev(line, nicos);
          }
        }
      }

      const func = applyFunc[p.type];
      if (!func) {
        return;
      }

      const beginTime = nicos.beginTime;
      const endTime = beginTime + nicos.duration;

      const members = Array.isArray(group) ? group : (group.members ?? (group as unknown as NicoChat[]));
      members.forEach((nicoChat) => {
        if (nicoChat.isNicoScript) {
          return;
        }
        const ct = nicoChat.beginTime;

        if (beginTime > ct || endTime < ct) {
          return;
        }
        func(nicoChat, nicos, p.params);
      });
    });
  }
}

//===END===

export { NicoScripter, NicoScriptParser };
/* eslint-disable */
//＠置換　U　「( ˘ω˘)ｽﾔｧ」 全
/* eslint-enable */
