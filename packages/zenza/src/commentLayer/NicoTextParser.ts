...
interface FontRegRecord {
  GOTHIC: RegExp;
  MINCHO: RegExp;
  GULIM: RegExp;
  MING_LIU: RegExp;
  GR: RegExp;
  STRONG_MINCHO: RegExp;
  BLOCK: RegExp;
}

interface ZenzaWatchLike {
  NicoTextParser?: unknown;
}

export type NicoTextParserType = InstanceType<ReturnType<typeof NicoTextParserInitFunc>>;
...
function NicoTextParserInitFunc() {
  class NicoTextParser {
    declare static _FONT_REG: FontRegRecord;
    declare static __css__: string;
    declare static likeXP: (text: string) => string;
    declare static likeHTML5: (text: string) => string;
  }
...
/* .line53 {
   ...
   */
  NicoTextParser.likeXP = (text: string): string => {
    const textUtil = util as unknown as TextUtilLike;
    const S = '<spacer> </spacer>';
    const ZS = '<spacer>全</spacer>';
    let htmlText = textUtil
      .escapeHtml(text)
    ...
    // 一文字ずつ走査してもさほど問題ないはず
    htmlText = htmlText.replace(NicoTextParser._FONT_REG.GR, (all: string, group: string, firstChar: string) => {
      // hasFontChanged = true;
      let baseFont: string;
      ...
      const result = ['<group class="', baseFont, ' fontChanged">', tmp.join(''), closer.join(''), '</group>'].join('');
    ...
    htmlText = htmlText
      ...
      .replace(
        /([\u2588]+)/g, //'<span class="fill_space">$1</span>')
      ...
      .replace(
        // eslint-disable-next-line no-misleading-character-class -- 非推奨空白文字の列挙であり結合文字の誤用ではない
        /([\uE800\u2002-\u200A\u007F\u05C1\u0E3A\u3164]+)/g,
      ...
      // 全角スペース
      .replace(
        /([\u3000]+)/g, //'<span class="zen_space type3000">$1</span>')
    ...
    htmlText = htmlText.replace(
      /(.)<group>([\u2001]+)<\/group>(.)/,
      '$1<group class="zen_space arial type2001">$2</group>$3'
    );
  ...
  NicoTextParser.likeHTML5 = (text: string): string => {
    const textUtil = util as unknown as TextUtilLike;
    let htmlText = textUtil
      .escapeHtml(text)
      .replace(/([\x20\xA0]+)/g, (g) => {
      ...
      .replace(/([\u2588]+)/g, (g) => {
        return (
          '<span class="html5_fill_space u2588">' + //g + '</span>';
          '田'.repeat(g.length) +
          '</span>'
        );
      })
...
(ZenzaWatch as unknown as ZenzaWatchLike).NicoTextParser = NicoTextParser;
//===END===
